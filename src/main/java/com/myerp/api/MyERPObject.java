...
import com.myerp.api.internal.guava.Iterators;
import com.myerp.api.internal.guava.Objects;
 ...
 * https://github.com/Ecwid/ecwid-mailchimp/blob/master/src/main/java/com/ecwid/mailchimp/MailChimpObject.java
 * -
 * Base class for all objects wrapping myERP API calls.
 * Since this object is generally a map, its mappings are used to serilize/deserialize the object to/from JSON.
 * <p>
 * Some entries in the map are <i>regular</i> mappings, whereas others are <i>reflective</i> ones. Reflective mappings
 * are represented by fields marked with the {@link Field} annotation. Changes in such fields are reflected in the map
 * and vice versa.
 * <p>
 ...
 */
public class MyERPObject extends AbstractMap<String, Object> implements Serializable {
  ...
  private static Map<Class<?>, Map<String, java.lang.reflect.Field>> reflectiveMappings =
      new ConcurrentHashMap<Class<?>, Map<String, java.lang.reflect.Field>>();
  private static Map<Class<?>, Map<String, Type>> reflectiveMappingsTypes =
      new ConcurrentHashMap<Class<?>, Map<String, Type>>();

  private static Map<String, java.lang.reflect.Field> getReflectiveMapping(Class<? extends MyERPObject> clazz) {
    ...
    if (mapping == null) {
      ...
      for (Class<?> c = clazz; MyERPObject.class.isAssignableFrom(c); c = c.getSuperclass()) {
	classes.addFirst(c);
      }
   ...
   */
  public static Map<String, Type> getReflectiveMappingTypes(Class<? extends MyERPObject> clazz) {
   ...
   *           if key is associated with a reflective mapping
   */
  @Override
  ...
  public Set<String> keySet() {
	...
	return o instanceof String && MyERPObject.this.containsKey((String) o);
      }

      @Override
      public boolean remove(Object o) {
	if (o instanceof String && MyERPObject.this.containsKey((String) o)) {
	  MyERPObject.this.remove((String) o);
	  return true;
	} else {
	  return false;
	}
  ...
  public Set<Map.Entry<String, Object>> entrySet() {
	...
	if (o instanceof Map.Entry) {
	  if (contains((Map.Entry) o)) {
	    MyERPObject.this.remove((String) ((Map.Entry) o).getKey());
	    return true;
	  } else {
	    return false;
	  }
	...
	if (o instanceof Map.Entry && ((Map.Entry) o).getKey() instanceof String) {
	  ...
	  if (MyERPObject.this.containsKey(key)) {
	    Object value1 = ((Map.Entry) o).getValue();
	    Object value2 = MyERPObject.this.get(key);
  ...
  public final String toJson() {
    return MyERPGsonFactory.createGson().toJson(this);
  }
   ...
   *
   * @throws IllegalArgumentException
   *           if <code>json</code> cannot be deserialized to an object of class <code>clazz</code>.
   */

  public static <T extends MyERPObject> T fromJson(String json, Class<T> clazz) {
    try {
      return MyERPGsonFactory.createGson().fromJson(json, clazz);
   ...
   */
  public <T extends MyERPObject> T as(Class<T> clazz) {
